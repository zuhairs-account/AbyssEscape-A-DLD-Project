...
def rom_12_bit(name, im, mask=False, rem_x=-1, rem_y=-1):
    ...
    for y in range(y_max):
        for x in range(x_max):
            # check if the color is 12'b000000000000
            color_bits = get_color_bits(im, y, x)
                # write : color_data =
            if color_bits != "000000000000":

                case = format(y, 'b').zfill(row_width) + format(x, 'b').zfill(col_width)
                f.write("\t\t" + str(row_width + col_width) + "'b" + case + ": color_data = " + str(12) + "'b" + color_bits + ";\n")
